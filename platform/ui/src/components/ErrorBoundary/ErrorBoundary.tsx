...
const DefaultFallback = ({ error, componentStack, context, resetErrorBoundary, fallbackRoute }) => {
  ...
  const title = `错误:${!isProduction && ` in ${context}`}.`;
  ...
  return (
    <div className="ErrorFallback bg-primary-dark w-full h-full" role="alert">
      ...
      <p className="text-primary-light text-base">{t(subtitle) as string}</p>
      {!isProduction && (
        <div className="rounded-md bg-secondary-dark p-5 mt-5">
          <pre className="text-primary-light">{t("Context")}: {context}</pre>
          <pre className="text-primary-light">{t("Error Message")}: {error.message}</pre>
          <pre className="text-primary-light">{t("Stack")}: {componentStack}</pre>
        </div>
      )}
    </div>
  );
