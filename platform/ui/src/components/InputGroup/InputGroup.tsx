...
const InputGroup = ({
  ...
  const renderFieldInputComponentLabel = ({
    name,
    displayName
  }) =>
    <InputLabelWrapper
  ...
  return (
    <div className="container m-auto relative flex flex-col">
      <div className="flex flex-row w-full">
        {inputMeta.filter(i => i.inputType !== "None").map((inputMeta, index) => {
          return (
            <div
              key={index}
      ...
      <div className="flex flex-row w-full" style={{ marginTop: '40px' }}>
        {inputMeta.map(inputMeta => {
          return (
            ...
            >
              {renderFieldInputComponentLabel(inputMeta)}
            </div>
          );
        })}
      </div>
